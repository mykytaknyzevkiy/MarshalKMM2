...
public class TriangulationClass {
	...
	*/
    ...
    static int Triangulate(int nv, List<Vector> pxyz, ITRIANGLE v[]) {
		...
		*/
		...
		/*
			...
			the triangle list.
		*/
        pxyz.get(nv + 0).x = xmid - 2.0 * dmax;
        pxyz.get(nv + 0).y = ymid - dmax;
        pxyz.get(nv + 0).z = 0.0;
    ...
    public void test(List<Vector> points) {
        {
            //System.out.println("Creating " + nv + " random points.");


            ITRIANGLE[] triangles = new ITRIANGLE[points.size() * 3];

            for (int i = 0; i < triangles.length; i++)
                triangles[i] = new ITRIANGLE();

            int ntri = Triangulate(points.size(), points, triangles);
		...
		*/
            ...
            for (int tt = 0; tt < points.size(); tt++) {
                System.out.println("rect(" + points.get(tt).x + "," + points.get(tt).y + ", 3, 3);");
            }
