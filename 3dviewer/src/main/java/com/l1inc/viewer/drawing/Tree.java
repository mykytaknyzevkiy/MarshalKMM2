...
public class Tree {
    ...
    private final float TreeSize = TREE_IGOLF_SIZE;
    ...
    public Tree(final Context context,
        ...
        if (polygonMap.get(textureResId3d) == null) {
            IndexedTexturedSquare polygon = new IndexedTexturedSquare(textureResId3d, context, 1.0f);
            polygonMap.put(textureResId3d, polygon);
        }

        if (polygonMap.get(shadowTextureResId) == null) {
            IndexedTexturedSquare polygon = new IndexedTexturedSquare(shadowTextureResId, context, 1.0f);
            polygonMap.put(shadowTextureResId, polygon);
        }

        if (polygonMap.get(textureResId2d) == null) {
            IndexedTexturedSquare polygon = new IndexedTexturedSquare(textureResId2d, context, 1.0f);
