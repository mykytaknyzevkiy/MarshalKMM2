...
import static android.opengl.GLES20.GL_COLOR_CLEAR_VALUE;
...
//            setHoleWithinCourse(holeWithinCourse, true, initialTeeBox);
        ...
        setTextureSet();
        if (elevationDataExist && ElevationHelper.getInstance().tilesExist() && TileStaticData.getInstance().dataExist()) {
...
//        }).start();
    ...
    private void loadHole_internal() {
//        Course3DRenderer.getViewerLogger().info(
//                String.format(Locale.US, "_rendering_ loadHole_internal thread id: %d, thread name: %s",
...
//                    Layer.PointInterpolation.Interpolate, ETC1Util.isETC1Supported());
            ...
            final Location overridedPinPosition = getOverridedPinPosition();

            try {

                if (pinPositionOverrideMap != null) {
                    float lat = (float) Layer.transformLat(overridedPinPosition.getLatitude());
                    float lon = (float) Layer.transformLon(overridedPinPosition.getLongitude());
            ...
            if (tree != null) {
                    ...
                    for (final Vector vector : pointList.getPointList()) {
                        ...
                        if (Course3DViewer.isLightweightRendering()) {
                            ...
                            for (final Tree tree : trees) {
                                if (tree.getPosition().distance(vector) < 0.5) {
                                    skipTree = true;
                                    skippedTreeCount++;
                                    break;
                                }
                ...
                Course3DRenderer.getViewerLogger().info("filtered trees count: " + skippedTreeCount);
            }
        ...
        Course3DRenderer.getViewerLogger().info("loaded hole in " + diff + "ms");
    ...
    public synchronized void onSurfaceChanged(final GL10 gl,
                                              ...
                                              final int height) {
//        Course3DRenderer.getViewerLogger().info(
//                String.format(Locale.US, "_rendering_ onSurfaceChanged thread id: %d, thread name: %s",
//                        Thread.currentThread().getId(), Thread.currentThread().getName())
//        );
        viewportWidth = width;
...
//        Course3DRenderer.getViewerLogger().info(
//                String.format(Locale.US, "_rendering_ renderToTextureAtPosition thread id: %d, thread name: %s",
//                        Thread.currentThread().getId(), Thread.currentThread().getName())
//        );
...
//        Course3DRenderer.getViewerLogger().info(
//                String.format(Locale.US, "_rendering_ onDrawFrame thread id: %d, thread name: %s",
//                        Thread.currentThread().getId(), Thread.currentThread().getName())
//        );
        ...
        if (needToClearGlBuffer) {
            needToClearGlBuffer = false;
            GLES20.glClear(GLES20.GL_COLOR_BUFFER_BIT | GLES20.GL_DEPTH_BUFFER_BIT);
            requestRender();
            return;
        }
        ...
        if (elevationDataExist) {
            if (!ElevationHelper.getInstance().tilesExist() || !TileStaticData.getInstance().dataExist()) {
                setCourseVectorDataMapInternal(courseVectorDataMap);
                needToChangeHole = true;
                onDrawFrame(gl);
                return;
            }
        ...
        if (tapCalled) {
            renderTap(tapXValue, tapYValue, viewAngle);
            tapCalled = false;
        }
...
//            }
        ...
        final Map<Integer, CartDrawData.CartMarker> cartMarkerArray;

        try {
            cartMarkerArray = new HashMap<Integer, CartDrawData.CartMarker>(cartDrawData.cartMarkerArray);
        } catch (Exception e) {
            e.printStackTrace();
            return;
        }
        ...
        for (Map.Entry<Integer, CartDrawData.CartMarker> entry : cartMarkerArray.entrySet()) {
    ...
    private boolean drawGroundAndHoleDataWithout3DBunkers(float x, float y, float z, float viewAngle, float rotationAngle) {
        ...
        if (navigationMode != NavigationMode.NavigationMode2D && elevationDataExist) {
            ...
            glUseProgram(shadowProgramId.getProgramId());
            bindShadowMatrix();
            ...
            Matrix.rotateM(intersectionUnprojectionMatrix, 0, viewAngle, 1, 0, 0);
            Matrix.translateM(intersectionUnprojectionMatrix, 0, 0, 0, -z);
            glUseProgram(programId.getProgramId());
            bindMatrix();
            Matrix.setIdentityM(modelViewMatrix, 0);
            Matrix.translateM(modelViewMatrix, 0, 0, 0, z);
            Matrix.rotateM(modelViewMatrix, 0, -viewAngle, 1, 0, 0);
...
//                bunker3DLayer.draw(this);
    ...
    public void tap(float x, float y) {
        if (elevationDataExist && ElevationHelper.getInstance().tilesExist() && navigationMode != null && navigationMode != NavigationMode.NavigationMode2D) {
        ...
        for (int i = 0; i < surfaceTriangle.length; i++) {
            ...
            if (result == 1) {
                ...
                if (elevationDataExist && ElevationHelper.getInstance().tilesExist()) {
                    ...
                    proceedAsyncTap(this.x, this.y, this.z, x, y, unprojected[0], unprojected[1], xpos, ypos, viewAngle, rotationAngle);
                    break;
                } else {
    ...
    private void proceedAsyncTap(final float cameraX, final float cameraY, final float cameraZ, final float tapX, final float tapY, final float unprojectedX, final float unprojectedY, final float xpos, final float ypos, final float viewAngle, final float rotationAngle) {
        new Thread(new Runnable() {
            public void run() {
                tapWithElevations(cameraX, cameraY, cameraZ, tapX, tapY, ElevationHelper.getInstance().getLeftBottomTapPosition(unprojectedX, unprojectedY, xpos, ypos, Course3DRenderer.this), viewAngle, rotationAngle);
                startTapDistanceMarkerHandler();
                tapDistanceCreateTime = System.currentTimeMillis();
                requestRender();
                viewerClickable = true;
    ...
    private void tapWithElevations(float cameraX, float cameraY, float cameraZ, float x, float y, int[] extremePoints, float viewAngle, float rotationAngle) {
        ...
        Matrix.setIdentityM(modelViewMatrix, 0);
        Matrix.translateM(modelViewMatrix, 0, 0, 0, cameraZ);
        Matrix.rotateM(modelViewMatrix, 0, -viewAngle, 1, 0, 0);
        Matrix.rotateM(modelViewMatrix, 0, rotationAngle, 0, 0, 1);
        Matrix.translateM(modelViewMatrix, 0, cameraX, cameraY, 0);

        float[] intersectionUnprojectionMatrix = new float[16];
        Matrix.setIdentityM(intersectionUnprojectionMatrix, 0);
        Matrix.translateM(intersectionUnprojectionMatrix, 0, -cameraX, -cameraY, 0);
        Matrix.rotateM(intersectionUnprojectionMatrix, 0, -rotationAngle, 0, 0, 1);
        Matrix.rotateM(intersectionUnprojectionMatrix, 0, viewAngle, 1, 0, 0);
        Matrix.translateM(intersectionUnprojectionMatrix, 0, 0, 0, -cameraZ);
        ...
        for (int y1 = extremePoints[0]; y1 < extremePoints[1]; y1++) {
            for (int x1 = extremePoints[2]; x1 < extremePoints[3]; x1++) {
                ...
                Matrix.translateM(retValMatrix, 0, 0, 0, cameraZ);
                Matrix.rotateM(retValMatrix, 0, -viewAngle, 1, 0, 0);
                Matrix.rotateM(retValMatrix, 0, rotationAngle, 0, 0, 1);
                Matrix.translateM(retValMatrix, 0, cameraX, cameraY, 0);
    ...
    private void bindSimpleShaders() {
        ...
        uMatrixLocation = GLES20.glGetUniformLocation(programId.getProgramId(), "u_Matrix");
        aPositionLocation = GLES20.glGetAttribLocation(programId.getProgramId(), "vPosition");
        vTexCoord = GLES20.glGetAttribLocation(programId.getProgramId(), "vTexCoord");
        texSampler = GLES20.glGetUniformLocation(programId.getProgramId(), "texSampler");
        opacity = GLES20.glGetUniformLocation(programId.getProgramId(), "Opacity");
        isTexture = GLES20.glGetUniformLocation(programId.getProgramId(), "isTexture");
        vColor = GLES20.glGetUniformLocation(programId.getProgramId(), "uColor");
    }

    public void bindShadowShaders() {

        shadowMatrixLocation = GLES20.glGetUniformLocation(shadowProgramId.getProgramId(), "u_Matrix");
        shadowPositionLocation = GLES20.glGetAttribLocation(shadowProgramId.getProgramId(), "vPosition");
        shadowTexCoord = GLES20.glGetAttribLocation(shadowProgramId.getProgramId(), "vTexCoord");
        shadowTexSampler = GLES20.glGetUniformLocation(shadowProgramId.getProgramId(), "texSampler");
        shadowOpacity = GLES20.glGetUniformLocation(shadowProgramId.getProgramId(), "Opacity");
        shadowIsTexture = GLES20.glGetUniformLocation(shadowProgramId.getProgramId(), "isTexture");
        shadowVColor = GLES20.glGetUniformLocation(shadowProgramId.getProgramId(), "uColor");

        normalLocation = GLES20.glGetAttribLocation(shadowProgramId.getProgramId(), "vNormal");
        modelViewMatrixLocation = GLES20.glGetUniformLocation(shadowProgramId.getProgramId(), "uMVMatrix");
        lightColor = GLES20.glGetUniformLocation(shadowProgramId.getProgramId(), "u_Light.Color");
        lightAmbientIntensity = GLES20.glGetUniformLocation(shadowProgramId.getProgramId(), "u_Light.AmbientIntensity");
        lightDiffuseIntensity = GLES20.glGetUniformLocation(shadowProgramId.getProgramId(), "u_Light.DiffuseIntensity");
        lightDirection = GLES20.glGetUniformLocation(shadowProgramId.getProgramId(), "u_Light.Direction");
        SpecularIntensity = GLES20.glGetUniformLocation(shadowProgramId.getProgramId(), "u_Light.SpecularIntensity");
        Shininess = GLES20.glGetUniformLocation(shadowProgramId.getProgramId(), "u_Light.Shininess");
