...
public interface VectorInt<T extends VectorInt<T>> {
    ...
    /** This method is faster than {@link VectorInt#len()} because it avoids calculating a square root. It is useful for comparisons,
    ...
    /** This method is faster than {@link VectorInt#dst(VectorInt)} because it avoids calculating a square root. It is useful for
    ...
    /** @return true if this vector is collinear with the other vector ({@link #isOnLine(VectorInt, float)} &&
     *         {@link #hasSameDirection(VectorInt)}). */
    boolean isCollinear (T other, float epsilon);

    /** @return true if this vector is collinear with the other vector ({@link #isOnLine(VectorInt)} &&
     *         {@link #hasSameDirection(VectorInt)}). */
    boolean isCollinear (T other);

    /** @return true if this vector is opposite collinear with the other vector ({@link #isOnLine(VectorInt, float)} &&
     *         {@link #hasOppositeDirection(VectorInt)}). */
    boolean isCollinearOpposite (T other, float epsilon);

    /** @return true if this vector is opposite collinear with the other vector ({@link #isOnLine(VectorInt)} &&
     *         {@link #hasOppositeDirection(VectorInt)}). */
