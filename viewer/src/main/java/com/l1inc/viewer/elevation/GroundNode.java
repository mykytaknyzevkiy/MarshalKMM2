...
public class GroundNode extends BaseDrawingObject {
    ...
    private void drawWithBuffers(Course3DRenderer renderer, int groundPos) {
        ...
        final ShortBuffer indices = TileStaticData.getInstance().getIndicies(mapHeight, mapWidth);
        if (indices != null) {
            GLES20.glDrawElements(GLES20.GL_TRIANGLES, indices.position(0).capacity() / 2, GLES20.GL_UNSIGNED_SHORT, 0);
        }
